...
int main(int argc, char* argv[]) {
  ...
  typedef std::shared_ptr<graComm::ServerBase> spServer;
  typedef std::shared_ptr<graComm::PkgBase> spPkg;
  ...
  lib1->addServer(spServer(new graComm::ModbusServer("127.0.0.1", 1502)));
  lib2->addServer(spServer(new graComm::ModbusServer("127.0.0.1", 1504)));

  std::fstream fs; // Create a ModbusPkg from file
  std::string cfgDIR = PROJECT_CONFIG_DIR;
  lib1->addPkg(spPkg(new graComm::ModbusPkg(fs, cfgDIR + "/ModbusConfig.txt")));
  lib1->addPkg(spPkg(new graComm::ModbusPkg(fs, cfgDIR + "/ModbusConfig.txt")));
  
  lib2->addPkg(spPkg(new graComm::ModbusPkg(fs, cfgDIR + "/ModbusConfig.txt")));
  lib2->addPkg(spPkg(new graComm::ModbusPkg(fs, cfgDIR + "/ModbusConfig.txt")));
